...
# ---------------------------------------------------------------------------
#  SYNCHRONIZED MOVE — master-wheel ramping keeps speed ratios locked
# ---------------------------------------------------------------------------

def _ramp_fraction(current, start, goal):
    """Velocity ramp fraction (0.1–1.0) based on position within the move.
    ...
    ramp_dist = max(total * RAMP_PCT, 0.3)

    if traveled < ramp_dist:
        return 0.1 + 0.9 * (traveled / ramp_dist)
    elif remaining < ramp_dist:
        return 0.1 + 0.9 * (remaining / ramp_dist)
    else:
        return 1.0
...
def _run_move(targets, vel_pct):
    ...
    All wheels are synchronized via a single master-wheel ramp so
    the speed ratios stay constant throughout the entire move.
    This keeps combined translate+rotate on the correct path.

    Old behavior: each wheel ramped independently based on its own
    travel distance — wheels with less travel finished early, breaking
    the mecanum vector during accel/decel phases.
    """
    global moving
    if not targets:
        moving = False
    ...
    # Peak velocity per wheel (encoder space), scaled so all finish together.
    # ratio = targets[nid] / max_travel gives a signed value in [-1, 1].
    # Multiply by dir to convert from logical to encoder space.
    peak = {nid: (targets[nid] / max_travel) * vel_scale * MOTORS[nid]["dir"]
            for nid in targets}

    start_pos = {nid: positions.get(nid, 0.0) for nid in targets}
    goal_pos  = {nid: start_pos[nid] + targets[nid] * MOTORS[nid]["dir"]
                 for nid in targets}

    master = max(targets, key=lambda nid: abs(targets[nid]))

    done = {nid: abs(targets[nid]) < 0.01 for nid in targets}
    start_time = time.time()
    tick = 0

    # Initial kick at 10% (bottom of ramp)
    for nid in targets:
        if not done[nid]:
            _command_vel(nid, peak[nid] * 0.1)

    while not all(done.values()):
        time.sleep(0.05)
        tick += 1

        # Single ramp fraction derived from the master wheel's position
        m_cur = positions.get(master, 0.0)
        frac = _ramp_fraction(m_cur, start_pos[master], goal_pos[master])
        ...
        for nid in targets:
            ...
            if abs(goal_pos[nid] - current) < POS_TOLERANCE:
                _command_vel(nid, 0.0)
                done[nid] = True
                print(f"  node {nid} ({MOTORS[nid]['role']}): reached ({current:.3f})")
            else:
                # Every wheel gets the SAME ramp fraction — ratios stay locked
                _command_vel(nid, peak[nid] * frac)
